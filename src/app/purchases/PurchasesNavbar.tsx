// components/PurchasesNavbar.tsx
'use client'
import Link from "next/link";
import { usePathname, useRouter } from "next/navigation";
import { List, Plus, ShoppingBag, FileText, ArrowLeft, ArrowRight } from "lucide-react";
import { Button, buttonVariants } from "@/components/ui/button";
import { ShineyButton } from "@/components/ShineyButton";
import MobileNav from "@/components/mobile-nav";

const PurchasesNavbar = () => {
  ...
  const navItems = [
    { label: "All Purchases", icon: <List />, path: "/purchases" },
    { label: "Add Purchase", icon: <Plus />, path: "/purchases/add" },
    { label: "Suppliers", icon: <ShoppingBag />, path: "/purchases/suppliers" },
    { label: "Reports", icon: <FileText />, path: "/purchases/reports" },
  ];
  ...
  return (
    <nav className="sticky mb-5 h-14 inset-x-0 top-0 z-30 w-full border-b border-gray-200 bg-white/75 backdrop-blur-lg transition-all">
      <div className="mx-auto h-14 px-6 flex justify-between items-center">
        <Link href="/purchases" className="flex z-40 font-semibold">
          ...
          <span className="text-blue-600 text-2xl font-mono">Purchases</span>
        </Link>
        <MobileNav />
